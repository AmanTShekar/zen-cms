...
describe('components Router Integration', () => {
  let app: any
  let zenith: Zenith
  let adapter: any
  ...
  beforeAll(async () => {
    ...
    try {
      const user = await adapter.create('users', { email: 'tester-components@test.com', password: 'abc', role: 'admin' })
      testUserId = user.id || user._id.toString()
      const ws = await adapter.create('z_workspaces', { slug: 'ws-components', name: 'WS', ownerId: testUserId })
      const site = await adapter.create('z_sites', { slug: 'site-components', name: 'Site', workspaceId: ws.id || ws._id.toString(), ownerId: testUserId })
      testSiteId = site.id || site._id.toString()

      adminToken = AuthService.generateToken({
        id: testUserId,
        email: 'tester-components@test.com',
        role: 'admin',
        siteId: testSiteId
      })
    } catch (e) {
