...
export const ZenithProvider: React.FC<ZenithProviderProps> = ({
  ...
  const client = useMemo(
    () => new ZenithClient({ baseURL, token, cacheTTL }),
    [baseURL, token, cacheTTL]
  )

  return <ZenithContext.Provider value={{ client }}>{children}</ZenithContext.Provider>
}
...
export const useZenithQuery = <T = any,>(
  ...
  React.useEffect(() => {
    ...
    // Optimization note: JSON.stringify is used here to deep-compare params. 
    // For peak performance in large apps, users should memoize the params object.
