...
router.post('/', async (req: Request, res: Response, next) => {
  try {
    ...
    const doc = await adapter.create('z_redirects', {
      ...
      siteId,
      hits: 0,
      createdBy: (req as import('express').Request & { user?: Record<string, any>, zenith?: Record<string, any> }).user?.id,
    })
...
router.patch('/:id', async (req: Request, res: Response, next) => {
  try {
    ...
    let existing: Record<string, any> = allById[0] || null
    if (!existing) {
    ...
    if (from && from !== existing.from) {
      ...
      const hasDup = dups.some((d: Record<string, any>) => String(d.id || d._id) !== req.params.id)
