...
const toCampaignDTO = (d: Record<string, unknown>) => ({
...
router.get('/', requireAuth, requireRole('admin'), async (req: Request, res: Response, next) => {
  try {
    ...
    const docs = await adapter.find<Record<string, unknown>>(CAMPAIGNS_COLLECTION, filter)
...
router.post('/', requireAuth, requireRole('admin'), async (req: Request, res: Response, next) => {
  try {
    ...
    const doc = await adapter.create<Record<string, unknown>>(CAMPAIGNS_COLLECTION, {
      subject,
      body: body || '',
      status: 'draft',
...
router.put('/:id', requireAuth, requireRole('admin'), async (req: Request, res: Response, next) => {
  try {
    ...
    const existing = await adapter.findOne<Record<string, unknown>>(CAMPAIGNS_COLLECTION, { _id: id, siteId })
    ...
    const doc = await adapter.update<Record<string, unknown>>(CAMPAIGNS_COLLECTION, id, update)
...
router.post('/:id/send', requireAuth, requireRole('admin'), async (req: Request, res: Response, next) => {
  try {
    ...
    const doc = await adapter.update<Record<string, unknown>>(CAMPAIGNS_COLLECTION, id, { 
      status: 'sent', 
      sent_at: new Date(),
      updated_at: new Date()
    })
