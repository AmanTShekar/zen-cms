...
const maskSiteSecrets = (site: Record<string, any> | null) => {
  if (!site) return site
  const copy = { ...site }
...
const unmaskSiteSecrets = (updates: Record<string, any>) => {
...
router.get('/', async (req: Request, res: Response, next) => {
  try {
    ...
    let sites = allSites.filter((s: Record<string, any>) =>
      user.role === 'admin' ||
      s.ownerId === user.id ||
      (Array.isArray(s.members) && s.members.some((m: Record<string, any>) => m.userId === user.id)) ||
      (Array.isArray((userFromDb as Record<string, any>).specialAccess) && (
        (userFromDb as Record<string, any>).specialAccess.includes(`site:${s.slug}`) ||
        (userFromDb as Record<string, any>).specialAccess.includes(`site:${s.id || s._id}`)
      ))
    )
    if (req.query.workspaceId) {
      sites = sites.filter((s: Record<string, any>) => s.workspaceId === req.query.workspaceId)
    }

    res.json(createResponse(sites.map((s: Record<string, any>) => maskSiteSecrets(s))))
  } catch (err) {
    next(err)
  }
})
...
router.get('/:id', async (req: Request, res: Response, next) => {
  try {
    ...
    let site: Record<string, any> | null = allById[0] || null
    if (!site) {
    ...
    const isMember = site && (
      ...
      (Array.isArray(site.members) && site.members.some((m: Record<string, any>) => m.userId === user.id))
    )
...
router.patch('/:id', async (req: Request, res: Response, next) => {
  try {
    ...
    const member = site.members.find((m: Record<string, any>) => m.userId === user.id)
...
router.post('/:id/members', async (req: Request, res: Response, next) => {
  try {
    ...
    const existingMember = site.members.find((m: Record<string, any>) => m.userId === userId)
    if (existingMember) {
...
router.delete('/:id/members/:userId', async (req: Request, res: Response, next) => {
  try {
    ...
    const filteredMembers = site.members.filter((m: Record<string, any>) => m.userId !== userId)
