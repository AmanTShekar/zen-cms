...
router.get(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      let settings = await adapter.findOne<Record<string, any>>('z_settings', query)
      if (!settings) {
        settings = await adapter.create<Record<string, any>>('z_settings', {
          siteName: 'Zenith CMS',
...
router.patch(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      if (!settings) {
        settings = await adapter.create<Record<string, any>>('z_settings', { ...unmaskedData, ...(siteId ? { siteId } : {}) })
      } else {
        console.log('PATCH SETTINGS:', settings); const id = (settings.id || settings._id).toString()
        settings = await adapter.update<Record<string, any>>('z_settings', id, unmaskedData)
      }
      res.json(createResponse(maskSettings(settings)))
    } catch (err) {
      next(err)
...
router.get(
  ...
  async (req: Request, res: Response, next) => {
    try {
      const adapter: DatabaseAdapter = (req as import('express').Request & { user?: Record<string, any>, zenith?: Record<string, any> }).zenith?.adapter || AdapterFactory.getActiveAdapter()
      const siteId = (req as import('express').Request & { user?: Record<string, any>, zenith?: Record<string, any> }).siteId || req.headers['x-zenith-site-id']
      const roles = await adapter.find<Record<string, any>>('z_roles', siteId ? { siteId } : {})
...
router.post(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      const existing = await adapter.findOne<Record<string, any>>('z_roles', { roleName, ...(siteId ? { siteId } : {}) })
...
router.get(
  ...
  async (req: Request, res: Response, next) => {
    try {
      const adapter: DatabaseAdapter = (req as import('express').Request & { user?: Record<string, any>, zenith?: Record<string, any> }).zenith?.adapter || AdapterFactory.getActiveAdapter()
      const siteId = (req as import('express').Request & { user?: Record<string, any>, zenith?: Record<string, any> }).siteId || req.headers['x-zenith-site-id']
      const users = await adapter.find<Record<string, any>>('users', siteId ? { siteId } : {})
      const sanitizedUsers = users.map((u: Record<string, any>) => {
...
router.patch(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      const { password, ...rest } = updated as Record<string, any>
...
router.post(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      const existingDb = await adapter.findOne<Record<string, any>>('z_collections', { slug: fileSlug, ...(siteId ? { siteId } : {}) })
      const existingStatic = engine?.config?.collections?.find((c: Record<string, any>) => c.slug === fileSlug)
...
router.patch(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      if (engine) {
        const colIdx = engine.config.collections.findIndex((c: Record<string, any>) => c.slug === slug)
        if (colIdx !== -1) {
...
router.post(
  ...
  async (req: Request, res: Response, next) => {
    try {
      ...
      const existing = await adapter.findOne<Record<string, any>>('users', { email: email.toLowerCase(), ...(siteId ? { siteId } : {}) })
      ...
      await adapter.create<Record<string, any>>('z_password_resets', { userId, token: resetToken, expiresAt })
