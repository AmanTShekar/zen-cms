...
describe('Zod Schema Engine', () => {
  it('should generate a schema for basic text fields', () => {
    ...
    expect(schema.parse({ title: 'Hello', description: 'World' })).toEqual({ title: 'Hello', description: 'World' });
  ...
  it('should handle array of strings using hasMany on select', () => {
    const fields: FieldConfig[] = [
      { name: 'tags', type: 'select', options: ['tech', 'design', 'business'], hasMany: true, required: true },
    ];
    const schema = createZodSchema(fields);
  ...
  it('should handle array of sub-document fields', () => {
    ...
    const schema = createZodSchema(fields);
    expect(schema.parse({ items: [{ name: 'Widget', qty: 3 }] })).toEqual({ items: [{ name: 'Widget', qty: 3 }] });
  ...
  it('should handle select with object options {label, value}', () => {
    const fields: FieldConfig[] = [
      { name: 'priority', type: 'select', options: [{ label: 'High', value: 'high' }, { label: 'Low', value: 'low' }], required: true },
    ];
    const schema = createZodSchema(fields);
