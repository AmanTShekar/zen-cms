...
export default function BlockBuilderPage() {
  ...
  const inputClass = cn(
    ...
    dark ? 'bg-black border-white/10 focus:border-emerald-500 text-white placeholder-gray-600' : 'bg-white border-gray-200 focus:border-emerald-500 text-gray-900 placeholder-gray-400'
  )
      ...
      <div className={cn('w-64 flex-shrink-0 border-r flex flex-col', dark ? 'border-white/5 bg-black' : 'border-gray-200 bg-white')}>
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          {loadingBlocks ? (
            ...
            <div className="text-center text-[9px] text-gray-500 uppercase tracking-widest italic">No blocks yet</div>
          ) : (
            Object.entries(blocksByCategory).map(([cat, blocks]) => (
              <div key={cat} className="space-y-1.5">
                ...
                <div className="space-y-0.5">
                  {blocks.map(block => (
                    <button key={block.slug} onClick={() => loadBlock(block)} className={cn('w-full flex items-center gap-2 text-left px-3 py-2 text-[10px] font-bold uppercase tracking-widest transition-colors rounded truncate', slug === block.slug ? 'bg-emerald-500 text-white' : dark ? 'text-gray-400 hover:bg-white/5 hover:text-white' : 'text-gray-600 hover:bg-gray-50')}>
                      {block.isGenerated ? <Code size={12} className={slug === block.slug ? 'text-white' : 'text-emerald-500/50'} /> : <Box size={12} />}
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative">
        <div className={cn('px-6 py-4 border-b flex items-center justify-between gap-4 shrink-0', dark ? 'border-white/5 bg-black' : 'border-gray-200 bg-white')}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-black uppercase italic tracking-tight">Component Builder</h1>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
                            ...
                            {field.i18n && <Globe size={10} className="text-blue-500" />}
                          </div>
            ...
            <motion.div initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }} className="w-full max-w-4xl border rounded-2xl shadow-2xl flex flex-col max-h-[85vh] bg-[#0a0f1a] border-white/10 overflow-hidden">
              ...
              {modalStep === 'SETTINGS' && (
                <div className="flex flex-col flex-1 overflow-hidden">
                  <div className="flex border-b border-white/10 px-6 pt-4 gap-6 bg-black/20">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                              ...
                              <div className="flex items-center">
                                <label className="flex items-center gap-2 cursor-pointer mt-4">
                                  <input type="checkbox" checked={!!activeField?.hasMany} onChange={e => setActiveField(prev => ({ ...prev, hasMany: e.target.checked }))} className="accent-emerald-500 w-4 h-4" />
                    ...
                    {settingsTab === 'VALIDATION' && (
                      <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4 bg-white/5 p-4 rounded-xl border border-white/5">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={!!activeField?.unique} onChange={e => setActiveField(prev => ({ ...prev, unique: e.target.checked }))} className="accent-emerald-500 w-4 h-4" />
                    ...
                    {settingsTab === 'ADMIN' && (
                      <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4 bg-white/5 p-4 rounded-xl border border-white/5">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={!!activeField?.admin?.hidden} onChange={e => setActiveField(prev => ({ ...prev, admin: { ...prev.admin, hidden: e.target.checked } }))} className="accent-emerald-500 w-4 h-4" />
                            <span className="text-xs font-bold text-gray-300">Hidden in UI</span>
                          </label>
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={!!activeField?.admin?.readOnly} onChange={e => setActiveField(prev => ({ ...prev, admin: { ...prev.admin, readOnly: e.target.checked } }))} className="accent-emerald-500 w-4 h-4" />
              ...
              <div className="px-6 py-4 border-t border-white/10 bg-black/50 flex justify-between">
                {modalStep === 'SETTINGS' ? (
