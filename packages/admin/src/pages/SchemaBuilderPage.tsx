...
const FIELD_TYPES = [
  ...
  { value: 'email', label: 'Email', icon: Globe, color: '#8b5cf6' },
  { value: 'password', label: 'Password', icon: Lock, color: '#ef4444' },
  { value: 'checkbox', label: 'Checkbox / Boolean', icon: ToggleLeft, color: '#10b981' },
  ...
  { value: 'slug', label: 'Slug', icon: Tag, color: '#10b981' },
  { value: 'array', label: 'Array / Repeater', icon: Layers, color: '#f59e0b' },
  { value: 'blocks', label: 'Dynamic Blocks', icon: Box, color: '#8b5cf6' },
    ...
    <div className="relative">
      <button
        ...
        className={cn(
          ...
          theme === 'dark' ? 'bg-black border-white/10 hover:border-emerald-500/50' : 'bg-gray-50 border-gray-200 hover:border-emerald-500'
        )}
      >
        ...
        {open && (
          <motion.div
            ...
            className={cn(
              ...
              theme === 'dark' ? 'bg-[#0a0f1a] border-white/10' : 'bg-white border-gray-200'
            )}
          >
            {FIELD_TYPES.map(ft => {
    ...
    <div className="mt-2">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className={cn('mt-3 p-4 border space-y-4', theme === 'dark' ? 'bg-black border-white/5' : 'bg-gray-50/50 border-gray-100')}>
              {/* Flags */}
...
${fieldsCode}
  ...
  const inputClass = cn(
    ...
    dark ? 'bg-black border-white/10 focus:border-emerald-500 text-white' : 'bg-white border-gray-200 focus:border-emerald-500 text-gray-900'
  )

  return (
    <div className={cn('flex h-[calc(100vh-4rem)] overflow-hidden', dark ? 'bg-[#080808] text-white' : 'bg-gray-50 text-gray-900')}>

      {/* ── Schemas Sidebar ─────────────────────────────────────────────── */}
      <div className={cn('w-64 flex-shrink-0 border-r flex flex-col', dark ? 'border-white/5 bg-black' : 'border-gray-200 bg-white')}>
            ...
            return (
              <div key={id} className="group flex items-center gap-1">
                ...
                >
                  ...
                  {schema.isCodeFirst && <Lock size={10} className={isActive ? 'text-white' : 'text-gray-500'} />}
                </button>
                {!schema.isCodeFirst && (
                  <button
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className={cn('px-6 py-4 border-b flex items-center justify-between gap-4 shrink-0', dark ? 'border-white/5 bg-black' : 'border-gray-200 bg-white')}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-black uppercase italic tracking-tight">Schema Builder</h1>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-purple-900/30"
            >
              <Sparkles size={14} /> AI Generate
            </button>
            <button
              onClick={() => setShowIntrospect(true)}
              className={cn('flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all border', dark ? 'border-white/10 hover:bg-white/5' : 'border-gray-200 hover:bg-gray-50')}
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-white text-[10px] font-black uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-emerald-900/30 disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="flex-1 overflow-auto p-6 space-y-6 custom-editor-scrollbar">
          ...
          <div className={cn('rounded-xl border p-6 space-y-4', dark ? 'bg-[#0a0f1a] border-white/[0.08]' : 'bg-white border-gray-100 shadow-sm')}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className={cn(inputClass, 'w-full rounded-lg', isCodeFirst && 'opacity-70 cursor-not-allowed')}
                  placeholder="e.g. Blog Posts"
                />
              </div>
              <div>
                <label className="text-[9px] font-black uppercase tracking-widest text-gray-500 block mb-1.5">Slug (API ID)</label>
                <input
                  type="text"
                  value={slug}
                  readOnly={isCodeFirst}
                  ...
                  className={cn(inputClass, 'w-full rounded-lg font-mono', isCodeFirst && 'opacity-70 cursor-not-allowed')}
                  placeholder="e.g. blog-posts"
                />
              </div>
            </div>
            ...
            {/* Settings toggles */}
            <div className="flex flex-wrap gap-x-6 gap-y-3 pt-2 border-t border-white/5">
              {(Object.keys(DEFAULT_SETTINGS) as (keyof CollectionSettings)[]).map(key => (
                <label key={key} className="flex items-center gap-2 cursor-pointer group">
                  ...
                  <span className="text-[10px] font-bold capitalize text-gray-400 group-hover:text-gray-200 transition-colors">
          ...
          <div className={cn('rounded-xl border', dark ? 'bg-[#0a0f1a] border-white/[0.08]' : 'bg-white border-gray-100 shadow-sm')}>
            <div className="px-6 py-4 border-b border-inherit flex items-center justify-between">
              ...
              {!isCodeFirst && (
                <button
                  onClick={addField}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-500 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all"
                >
            ...
            <div className="p-4 space-y-3">
              <AnimatePresence>
                {fields.map((field, i) => (
                  <motion.div
                    ...
                    className={cn('p-4 border rounded-xl relative group', dark ? 'bg-black/40 border-white/5' : 'bg-gray-50 border-gray-100', isCodeFirst && 'opacity-70')}
                  >
                    {/* Delete button */}
                    {!isCodeFirst && (
                      <button
                        onClick={() => removeField(i)}
                        className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-1.5 text-red-500/50 hover:text-red-500 transition-all rounded-lg hover:bg-red-500/10"
                      >
                        <Trash2 size={14} />
                      </button>
                    )}
                    ...
                    <div className="grid grid-cols-3 gap-3 pr-8">
                      <div>
                        ...
                        <input
                          ...
                          className={cn(inputClass, 'w-full rounded-lg font-mono text-[11px]', isCodeFirst && 'cursor-not-allowed')}
                          placeholder="fieldName"
                        />
                      </div>
                      <div>
                        <label className="text-[9px] font-black uppercase tracking-widest text-gray-500 block mb-1.5">Field Type</label>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className={cn(inputClass, 'w-full rounded-lg', isCodeFirst && 'cursor-not-allowed')}
                    ...
                    {/* Quick flags row */}
                    <div className="flex items-center gap-5 mt-3 pt-2 border-t border-white/[0.04]">
              ...
              {fields.length === 0 && (
                <div className="py-16 text-center">
                  ...
                  <p className="text-[11px] font-black uppercase tracking-widest text-gray-600 italic">No fields yet</p>
      ...
      <AnimatePresence>
        {showCode && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="w-full max-w-3xl border rounded-xl shadow-2xl p-6 relative flex flex-col h-[80vh] bg-[#0B0F19] border-white/10">
              ...
              <div className="flex-1 bg-[#1E1E1E] border border-white/10 p-5 overflow-auto text-sm font-mono text-emerald-400 relative rounded-lg">
                <button
                  onClick={() => { navigator.clipboard.writeText(generateCode()); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
                  className="absolute top-3 right-3 p-2 bg-white/10 hover:bg-white/20 text-white transition-colors rounded-lg"
                >
      ...
      <AnimatePresence>
        {showAI && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="w-full max-w-lg border rounded-xl shadow-2xl p-6 relative bg-[#0B0F19] border-white/10">
              ...
              <textarea
                ...
                className="w-full bg-black border border-white/10 focus:border-purple-500/50 p-4 text-sm outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:ring-offset-1 focus-visible:ring-offset-black rounded-lg placeholder:text-gray-600 text-white resize-none mb-4"
              />
              <button
                disabled={isAIGenerating || !aiPrompt}
                onClick={handleAIGenerate}
                className="w-full py-3.5 bg-purple-600 hover:bg-purple-500 text-white text-[10px] font-black uppercase tracking-widest flex justify-center items-center gap-2 transition-all rounded-lg disabled:opacity-50"
              >
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="w-full max-w-lg border rounded-xl shadow-2xl p-6 relative bg-[#0B0F19] border-white/10">
              ...
              <input
                ...
                className="w-full bg-black border border-white/10 focus:border-emerald-500/50 p-3.5 text-sm outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:ring-offset-1 focus-visible:ring-offset-black rounded-lg placeholder:text-gray-600 text-white mb-4 font-mono"
              />
              <button
                disabled={isIntrospecting}
                onClick={handleIntrospect}
                className="w-full py-3.5 bg-emerald-500 hover:bg-emerald-400 text-white text-[10px] font-black uppercase tracking-widest flex justify-center items-center gap-2 transition-all rounded-lg"
              >
