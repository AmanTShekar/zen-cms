...
import { Sparkles, Send, Loader2, Copy, Zap, Terminal, Cpu, MessageSquare, PenTool, Type, Globe, ShieldCheck, Search, Image as ImageIcon, CheckCircle2, AlertCircle } from 'lucide-react';
      ...
      return (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase tracking-[0.4em] italic text-gray-500">SEO Results</span>
            <span className={cn(
          ...
          <div className="space-y-3">
            {result.issues?.map((issue: string, i: number) => (
              <div key={i} className="flex gap-3 items-start p-3 bg-white/5 border border-white/5 rounded-none">
                <AlertCircle size={14} className="text-amber-500 shrink-0 mt-0.5" />
                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tight leading-relaxed italic">{issue}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-white/5 border border-white/5 rounded-none">
              <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest block mb-1">Readability</span>
              <span className="text-xl font-black text-indigo-500 italic uppercase tracking-tighter">{result.readability}</span>
            </div>
            <div className="p-4 bg-white/5 border border-white/5 rounded-none">
              <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest block mb-1">Complexity</span>
              <span className="text-xl font-black text-emerald-500 italic uppercase tracking-tighter">{result.complexity}</span>
            </div>
          </div>
          <div className="space-y-2">
            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest italic block">Suggestions</span>
            {result.suggestions?.map((s: string, i: number) => (
              <div key={i} className="flex gap-3 items-center p-2">
                <CheckCircle2 size={12} className="text-indigo-500" />
                <span className="text-[9px] font-bold text-gray-400 uppercase tracking-tight italic">{s}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-10">
         <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
               <h1 className="text-4xl font-black tracking-tighter uppercase italic leading-none">AI Content Architect</h1>
               <div className="flex items-center gap-3 mt-2">
                  <div className="px-3 py-1 rounded-none bg-white/5 border border-white/5 text-[8px] font-black text-indigo-400 uppercase italic">Zenith Pro</div>
         ...
         <div className={cn(
           ...
           theme === 'dark' ? "bg-white/[0.05] border-white/5" : "bg-white border-gray-100 shadow-sm"
         )}>
            {[
              ...
              >
                ...
                <div className={cn(
                  ...
                  theme === 'dark' ? "bg-indigo-500/5 border-indigo-500/20" : "bg-indigo-50 border-indigo-100"
                   ...
                   <div className="flex items-center gap-4">
                      ...
                      <span className="text-[12px] font-black uppercase tracking-widest italic">Prompt Examples</span>
                   </div>
                   <p className="text-[11px] font-bold text-gray-500 uppercase tracking-tight leading-relaxed italic">
                     ...
                     <span className="text-indigo-400">"Generate a 5-step workflow for a multi-tenant SaaS media pipeline"</span>
                   </p>
                </div>
                ...
                {mode === 'tools' && (
                  <div className={cn(
                    ...
                    <span className="text-[9px] font-black text-gray-500 uppercase tracking-[0.4em] px-2 italic">Select Tool</span>
                      ...
                      {tools.map(tool => (
                        <button
                          ...
                          className={cn(
                            ...
                            activeTool === tool.id 
                              ? (theme === 'dark' ? "bg-white border-white text-black" : "bg-gray-900 text-white shadow-lg") 
                  ...
                  theme === 'dark' ? "bg-[#080808] border-white/5" : "bg-white border-gray-100"
                   ...
                   <div className="flex items-center gap-4 mb-6">
                      <div className={cn(
                        ...
                        theme === 'dark' ? "bg-white/5 border-white/10 text-indigo-500" : "bg-gray-50 border-gray-100 text-indigo-600"
                      )}>
                         <Terminal size={16} />
                      </div>
                      <div className="flex flex-col">
                         <span className="text-[10px] font-black uppercase italic tracking-tight">Input</span>
                         <span className="text-[7px] font-bold text-gray-500 uppercase tracking-widest mt-1">Ready for data</span>
                      </div>
                   </div>
                   ...
                   <textarea 
                      ...
                      placeholder={
                        mode === 'tools' ? (activeTool === 'alt' ? 'Insert image URL...' : 'Paste text to analyze...') : 
                        mode === 'content' ? "Describe content to synthesize..." : "Define schema structure..."
                      }
                      className={cn(
                        ...
                        theme === 'dark' ? "border-white/10 text-white focus:border-indigo-500/30" : "border-gray-100 text-gray-900 focus:border-indigo-500/20"
                      )}
                   />
                   ...
                   <button 
                      ...
                      className={cn(
                        ...
                        loading ? "opacity-50 cursor-not-allowed" : (theme === 'dark' ? "bg-white text-black hover:bg-gray-200" : "bg-gray-900 text-white hover:bg-black")
                      )}
                   >
         ...
         <div className="flex-1 relative">
            <button 
              ...
              className={cn(
                ...
                theme === 'dark' ? "bg-black border-white/10 text-gray-500 hover:text-white" : "bg-white border-gray-100 text-gray-400 hover:text-indigo-600 shadow-md"
              )}
            >
              <div className={cn("w-1 h-4 rounded-none bg-current transition-transform", isInputVisible ? "" : "rotate-180")} />
            </button>
            <div className={cn(
              ...
              theme === 'dark' ? "bg-[#080808]/80 border-white/5" : "bg-white border-gray-100"
                 ...
                 theme === 'dark' ? "bg-white/[0.02] border-white/5" : "bg-gray-50/30 border-gray-100"
                  ...
                  <div className="flex items-center gap-4">
                     <div className={cn(
                       ...
                       theme === 'dark' ? "bg-white/5 border-white/10 text-emerald-500" : "bg-white border-gray-100 text-emerald-600"
                     )}>
                        <Type size={14} />
                     </div>
                     <span className="text-[10px] font-black uppercase italic tracking-widest">Output</span>
                  </div>
                  {result && (
                    <button 
                      onClick={() => {
                      ...
                      className={cn(
                        ...
                        theme === 'dark' ? "hover:bg-white/5 text-gray-500 hover:text-white" : "hover:bg-gray-50 text-gray-400 hover:text-indigo-600"
                      )}
                    >
                       <Copy size={16} />
                    </button>
               ...
               <div className="flex-1 p-8 overflow-auto font-mono text-[11px] font-black italic text-gray-500 leading-relaxed selection:bg-indigo-500 selection:text-white custom-scrollbar relative">
                  {mode === 'structure' && !result && !loading && (
                    <div className="h-full flex flex-col p-10 space-y-12">
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black uppercase tracking-[0.4em] italic text-indigo-500">Persist_Canvas</span>
                          <span className="text-[8px] font-bold text-gray-500 uppercase tracking-widest mt-2">Active_Spatial_Registry</span>
                        </div>
                        <div className="flex items-center gap-4">
                          <div className="flex flex-col items-end">
                            <span className="text-[7px] font-black uppercase tracking-widest text-gray-600">Node_Density</span>
                            <span className="text-[14px] font-black italic">0%</span>
                          </div>
                          <div className="w-32 h-1 bg-white/5 rounded-none overflow-hidden">
                            <div className="w-0 h-full bg-indigo-500 shadow-[0_0_10px_#6366f1]" />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 border border-white/[0.03] rounded-none bg-white/[0.01] relative overflow-hidden flex items-center justify-center group">
                        ...
                        <div className="flex flex-col items-center gap-6 group-hover:scale-110 transition-transform duration-700 opacity-20">
                          <Globe size={64} strokeWidth={1} className="text-indigo-500" />
                          <span className="text-[9px] font-black uppercase tracking-[0.6em] italic">Sections_Map_Standby</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {mode !== 'structure' && !result && !loading && (
                     <div className="h-full flex flex-col items-center justify-center gap-6 opacity-10">
                        ...
                        <span className="text-[10px] uppercase tracking-[0.5em]">Waiting for input...</span>
                     </div>
                  )}
                  {loading && (
                     <div className="h-full flex flex-col items-center justify-center gap-8">
                        <div className="relative">
                          <Loader2 size={40} className="animate-spin text-indigo-500 opacity-40" />
                          <div className="absolute inset-0 bg-indigo-500/20 blur-xl rounded-none animate-pulse" />
                        </div>
                        <span className="text-[10px] uppercase tracking-[0.5em] animate-pulse">Thinking...</span>
                     </div>
                  )}
                  {result && !loading && (
                     ...
                     <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                           ...
                           <div key={i} className="p-3 rounded-none bg-white/5 border border-white/5">
                              <span className="text-[7px] font-black uppercase tracking-widest text-gray-500 block mb-1">{stat.label}</span>
                              <span className="text-[10px] font-black uppercase italic text-indigo-400">{stat.value}</span>
                           </div>
                        ))}
                     </div>
                     <button 
                        className={cn(
                          ...
                          theme === 'dark' ? "bg-indigo-500 text-white hover:bg-indigo-600 shadow-lg shadow-indigo-500/10" : "bg-indigo-600 text-white hover:bg-indigo-700"
                        )}
