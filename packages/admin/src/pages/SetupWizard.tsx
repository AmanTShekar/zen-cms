...
export default function SetupWizard() {
  ...
  const testDbConnection = async () => {
    ...
    try {
      ...
      const msg = e.response?.data?.error?.message || e.message || 'Database verification failed'
  ...
  const saveDbConnection = async () => {
    try {
      ...
      const msg = e.response?.data?.error?.message || e.message || 'Database save failed'
      toast.error(msg)
    }
  }
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {PROJECT_TYPES.map((pt) => (
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase italic">{pt.label}</span>
            </button>
          ))}
        </div>
      ),
      ...
      content: (
        <div className="space-y-3">
          ...
          {state.selectedCollections.length === 0 && (
            <p className="text-[9px] text-amber-500 font-black uppercase italic text-center py-2">
      ...
      content: (
        <div className="space-y-4">
          ...
          {state.dbTestStatus !== 'idle' && (
            <div
              className={cn(
                'p-4 border rounded-none text-[10px] font-bold uppercase tracking-wide italic',
        ...
        <div className="space-y-5">
          {!state.generatedKey ? (
            ...
            <div className="space-y-4">
              ...
              >
                <p className="text-[9px] font-black uppercase text-emerald-500 italic">
  ...
  return (
    <div className={cn('min-h-screen flex flex-col items-center justify-center p-4', bg)}>
      ...
      <div className="w-full max-w-2xl flex justify-end mb-4">
        <button
          onClick={skip}
          className="flex items-center gap-1 text-[9px] font-black uppercase text-gray-500 hover:text-gray-300 transition-colors italic"
        >
      ...
      <div className={cn('w-full max-w-2xl border rounded-none overflow-hidden', card)}>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-[22px] font-black uppercase italic leading-tight">
                  {current.title}
                </h2>
        ...
        >
          <button
            ...
            disabled={step === 0}
            className={cn(
              'flex items-center gap-2 px-5 py-2.5 border text-[10px] font-black uppercase italic rounded-none transition-all',
              step === 0
          ...
          {step === TOTAL_STEPS - 1 ? (
            <button
              onClick={() => navigate('/')}
              className="flex items-center gap-2 px-8 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-black uppercase italic rounded-none transition-all shadow-lg shadow-emerald-600/20"
            >
