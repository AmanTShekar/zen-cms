...
const SettingsDatabase: React.FC<SettingsDatabaseProps> = ({ dbStats, theme }) => {
  ...
  return (
    <div className="col-span-full space-y-8">
        ...
        {stats.map((stat, i) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className={cn('text-[10px] font-black uppercase tracking-widest italic', stat.color)}>
                Synchronized
              </span>
            </div>
      ...
      >
        <p className="text-[9px] font-black uppercase italic tracking-[0.3em] text-gray-500 mb-4">
          Test New Connection
        </p>
        <form onSubmit={handleTestConnection} className="flex gap-3 items-end">
          <div className="flex-1 space-y-1">
            <label className="text-[8px] font-black uppercase italic tracking-widest text-gray-600">
              Connection URI
            </label>
            <input
              type="text"
            ...
            <div className="flex gap-2">
              {(['postgres', 'mongodb'] as const).map((d) => (
                <button
                  ...
                  className={cn(
                    'px-4 py-3 text-[9px] font-black uppercase italic border rounded-none transition-all',
                    dbDialect === d
      ...
      <div className="flex flex-wrap gap-4">
        <button
          ...
          disabled={sweeping}
          className="flex items-center gap-3 px-8 py-4 rounded-none bg-red-500/10 text-red-500 border border-red-500/20 text-[10px] font-black uppercase italic hover:bg-red-500/20 transition-all active:scale-95 disabled:opacity-40"
        >
        ...
        <button
          ...
          disabled={sweeping}
          className="flex items-center gap-3 px-8 py-4 rounded-none bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 text-[10px] font-black uppercase italic hover:bg-emerald-500/20 transition-all active:scale-95 disabled:opacity-40"
        >
