...
const NestedArrayEditor: React.FC<NestedArrayEditorProps> = ({
  label,
  items = [],
  onChange,
  theme,
  ...
  const handleAddItem = () => {
    if (items.length >= 10) {
      toast.error('Maximum 10 items reached')
      return
    }
    ...
    if (Object.keys(firstItem).length > 0) {
      ...
      if (lower.includes('plan')) {
        newItem.name = 'New Plan'
        newItem.price = '$19/mo'
        newItem.features = 'Basic Features'
      } else if (lower.includes('feature')) {
        newItem.title = 'New Feature'
        newItem.description = 'Feature description text.'
      } else if (lower.includes('member') || lower.includes('team')) {
        newItem.name = 'New Team Member'
        newItem.role = 'Contributor'
        newItem.bio = 'Bio details...'
      } else if (lower.includes('faq') || lower.includes('item')) {
        newItem.question = 'New Question?'
        newItem.answer = 'Answer details.'
      } else {
    ...
    toast.success('Added new card item', { icon: '✨' })
  }
  ...
  return (
    <div
      className={cn(
        'p-4 border rounded-none space-y-4 md:col-span-2 w-full',
        theme === 'dark'
    ...
    >
      <div className="flex items-center justify-between border-b border-white/5 pb-2">
        ...
        <button
          ...
          className={cn(
            'px-3 py-1 text-[8px] font-black uppercase italic rounded-none border transition-all flex items-center gap-1',
            theme === 'dark'
      ...
      {items.length === 0 ? (
        <div className="py-6 text-center text-[10px] text-gray-500 font-bold uppercase italic tracking-widest">
          No cards in this collection. Click Add Item to start!
        </div>
      ) : (
          ...
          {items.map((item, index) => {
            ...
            return (
              <div
                key={index}
                className={cn(
                  'p-4 border rounded-none flex flex-col justify-between space-y-3 relative group/card',
                  theme === 'dark'
                    ? 'bg-black border-white/5 hover:border-indigo-500/30'
                    : 'bg-white border-gray-200 hover:border-indigo-200 shadow-sm'
                )}
              >
                <div className="flex items-center justify-between border-b border-white/5 pb-1">
                  <span className="text-[9px] font-black uppercase italic text-gray-400">
                    💳 Card #{index + 1}:{' '}
                ...
                <div className="space-y-2">
                  {Object.keys(item).map((itemKey) => {
                    if (itemKey === '_id' || itemKey === 'id') return null
                    return (
                      <div key={itemKey} className="space-y-1">
                        <label className="text-[7px] font-black text-gray-500 uppercase tracking-widest block italic">
                          {humanize(itemKey)}
                        </label>
                        <input
                          type="text"
                          value={item[itemKey] || ''}
                          onChange={(e) => handleItemChange(index, itemKey, e.target.value)}
                          className={cn(
                            'w-full rounded-none py-1.5 px-2.5 text-[10px] font-bold outline-none border transition-all',
                            theme === 'dark'
                          ...
                          placeholder={`Enter ${humanize(itemKey)}...`}
                        />
                      </div>
                    )
                  })}
...
const SpatialEditor: React.FC<{ isGlobal?: boolean }> = ({ isGlobal }) => {
  ...
  // Live_Preview_Sync_Kernel
  useEffect(() => {
    if (iframeRef.current?.contentWindow && data && !loading) {
      iframeRef.current.contentWindow.postMessage(
        {
          type: 'ZENITH_DATA_UPDATE',
          data: data,
        },
        '*'
      )
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const normalizedSections = (res.data.data.sections || []).map((s: any) => ({
          ...s,
          id: s.id || `node_${nodeCounter.current++}`,
  ...
  const removeSection = (id: string) => {
    setData((prev: any) => ({
      ...prev,
      sections: Array.isArray(prev?.sections) ? prev.sections.filter((s: any) => s.id !== id) : [],
    }))
  ...
  return (
    ...
    >
      ...
      <header
        className={cn(
          'h-16 border-b flex items-center justify-between px-8 z-[100] backdrop-blur-3xl transition-all',
          theme === 'dark' ? 'bg-black/80 border-white/5' : 'bg-white/80 border-gray-100 shadow-sm'
        )}
      >
        <div className="flex items-center gap-8">
          <button
            onClick={() => navigate(-1)}
            className="p-2 hover:text-indigo-500 transition-colors bg-white/5 rounded-none border border-white/5"
          >
            <ChevronLeft size={20} />
          </button>
          <div className="flex flex-col">
            <span className="text-[12px] font-black tracking-[0.5em] text-indigo-500 uppercase italic leading-none">
              Zenith Editor
            </span>
        ...
        <div className="flex items-center gap-6">
              ...
              className={cn(
                ...
                leftPanelOpen
                  ...
                  : theme === 'dark'
                    ...
                    : 'bg-gray-100 border-gray-200 text-gray-400 hover:text-black'
              )}
            >
              <PanelLeft size={18} />
            </button>
          ...
          <div
            className={cn(
              'flex items-center gap-1 p-1 rounded-none border',
          ...
          >
            <button
              ...
              className={cn(
                'px-4 py-1.5 rounded-none text-[10px] font-black uppercase italic transition-all',
                viewMode === 'visual'
                  ? theme === 'dark'
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-2 px-4 py-2 rounded-none border transition-all text-[10px] font-black uppercase italic',
              seoOpen
                ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-400'
                : theme === 'dark'
                  ? 'bg-white/5 border-white/5 text-gray-400 hover:text-white'
                  : 'bg-gray-50 border-gray-200 text-gray-500 hover:text-black shadow-sm'
            )}
          >
            <Globe size={16} /> SEO
          </button>
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-3 px-6 py-2 rounded-none text-[10px] font-black uppercase bg-indigo-600 text-white shadow-[0_0_20px_rgba(79,70,229,0.4)] hover:bg-indigo-500 transition-all active:scale-95"
          >
        ...
        <AnimatePresence initial={false}>
          {leftPanelOpen && (
            <motion.aside
              ...
              className={cn(
                'border-r flex flex-col z-50 overflow-hidden transition-[width] duration-75 ease-out shrink-0 relative group/sidebar',
                theme === 'dark'
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  className={cn(
                    'w-6 h-6 rounded-none flex items-center justify-center transition-all shadow-sm border',
                    theme === 'dark'
                      ? 'bg-white/5 border-white/5 text-white hover:bg-indigo-600'
                      : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-indigo-600 hover:text-white'
                  )}
                >
                  <Plus size={14} />
                </button>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar px-2 space-y-4 pt-4 custom-editor-scrollbar">
                <div className="space-y-2">
                  <div className="flex items-center justify-between px-2">
                    <span className="text-[9px] font-black text-gray-500 uppercase tracking-[0.3em] italic opacity-50">
                      Editor Root
                    </span>
                  </div>
                  <button
                    onClick={() => {
                      ...
                      document
                        .getElementById('document-kernel')
                        ?.scrollIntoView({ behavior: 'smooth' })
                    }}
                    className={cn(
                      'w-full flex items-center gap-3 p-3 rounded-none border transition-all duration-200',
                      activeSection === 'root'
                        ? theme === 'dark'
                ...
                <div className="space-y-2 pb-20">
                  <div className="flex items-center justify-between px-2">
                    ...
                    <span className="text-[8px] font-black text-white italic opacity-10">
                      {data?.sections?.length || 0} Layers
                    </span>
                  </div>
                  <Reorder.Group
                    axis="y"
                    ...
                    {data?.sections?.map((section: Section) => (
                      <Reorder.Item key={section.id} value={section} className="relative group">
                        <button
                          ...
                          className={cn(
                            'w-full flex items-center gap-3 p-2.5 rounded-none border transition-all duration-200',
                        ...
                        >
                          ...
                          <div className="flex-1 text-left overflow-hidden">
                            <span className="text-[9px] font-black uppercase italic tracking-tight block truncate">
            ...
            <div className="max-w-[1400px] mx-auto min-h-screen flex flex-col">
              {viewMode === 'visual' ? (
                <div className="flex-1 space-y-12">
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {(['left', 'center', 'right'] as const).map((align) => (
                          <button
                            key={align}
                            onClick={() => data && setData({ ...data, align })}
                            className={cn(
                              'p-1 transition-all',
                              data?.align === align || (!data?.align && align === 'left')
                                ? theme === 'dark'
                    ...
                    <div className="relative group/title">
                      <RichTextEditor
                        mode="heading"
                        value={data?.title || ''}
                        onChange={(val) => data && setData({ ...data, title: val })}
                    ...
                    <div className="space-y-4">
                      <div
                        className={cn(
                          ...
                          theme === 'dark' ? 'bg-white/[0.01]' : 'bg-gray-50 shadow-inner'
                        )}
                      >
                        <RichTextEditor
                          mode="full"
                          value={data?.heroDescription || ''}
                          onChange={(val) => data && setData({ ...data, heroDescription: val })}
                        />
                      </div>
                    </div>
                  </div>
                    ...
                    {data?.sections?.map((section: Section, idx: number) => (
                      ...
                      >
                        <div className="relative h-4 group/portal -mt-2">
                          <button
                            onClick={() => {
                              setInjectionIndex(idx)
                              setBlockPickerOpen(true)
                            }}
                            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover/portal:opacity-100 transition-all z-20"
                          >
                            <div
                              className={cn(
                                'px-4 py-1 rounded-none border backdrop-blur-xl text-[8px] font-black uppercase italic flex items-center gap-2',
                                theme === 'dark'
                        ...
                        <div
                          ...
                          className={cn(
                            'p-6 rounded-none border transition-all duration-500 relative group/section',
                            activeSection === section.id
                              ? theme === 'dark'
                                ...
                                return (
                                  <NestedArrayEditor
                                    ...
                                    onChange={(newVal) => {
                                      ...
                                      newSections[sIdx].content[key] = newVal
                                      setData({ ...data, sections: newSections })
                                    }}
                                  />
                                )
                                ...
                                >
                                  ...
                                  {key.toLowerCase().includes('image') ? (
                                    <MediaPicker
                                      ...
                                      onChange={(v) => {
                                        const newSections = [...data.sections]
                                        const sIdx = newSections.findIndex(
                                          (s) => s.id === section.id
                                        )
                                        newSections[sIdx].content[key] = v
                                        setData({ ...data, sections: newSections })
                                      }}
                                    />
                                  ) : (
                  ...
                  <button
                    onClick={() => {
                      setInjectionIndex(data?.sections?.length || 0)
                      setBlockPickerOpen(true)
                    }}
                    className={cn(
                      'w-full py-10 rounded-none border-2 border-dashed transition-all flex flex-col items-center gap-4 group',
                      theme === 'dark'
                        ? 'border-white/5 hover:border-indigo-500/20 hover:bg-white/[0.01]'
                        : 'border-gray-100 hover:border-indigo-500/20 hover:bg-gray-50'
                    )}
                  >
                    <div
                      className={cn(
                        'w-10 h-10 rounded-none border flex items-center justify-center group-hover:scale-110 transition-all',
                        theme === 'dark'
                          ? 'bg-white/5 border-white/5'
                          : 'bg-gray-100 border-gray-200'
                      )}
                    ...
                    <p className="text-[11px] font-black uppercase tracking-[0.3em] italic text-gray-500 group-hover:text-indigo-400">
                      Append Section
                    </p>
                  </button>
                </div>
                ...
                <div className="flex-1 flex flex-col pt-10">
                  <div
                    className={cn(
                      'flex-1 p-10 font-mono text-sm overflow-auto rounded-none border',
                      theme === 'dark'
                        ? 'bg-black/50 border-white/5 text-indigo-300'
                        : 'bg-gray-100 border-gray-200 text-indigo-900'
                    )}
                  >
        ...
        <AnimatePresence initial={false}>
          {rightPanelOpen && (
            <motion.aside
              ...
              exit={{ width: 0, opacity: 0 }}
              className={cn(
                'border-l flex flex-col z-50 overflow-hidden shrink-0 relative group/right-sidebar',
                theme === 'dark'
                  ? 'bg-[#080808] border-white/5'
                  : 'bg-white border-gray-200 shadow-xl'
              )}
                  ...
                  {(['preview', 'history'] as const).map((tab) => (
                    <button
                      ...
                      className={cn(
                        ...
                        activeRightTab === tab ? 'opacity-100' : 'opacity-30 hover:opacity-100'
                      )}
                    >
                      <span
                        className={cn(
              ...
              <div className="flex-1 overflow-hidden relative custom-editor-scrollbar">
                {activeRightTab === 'preview' ? (
                  <div className="w-full h-full flex flex-col">
                    <div
                      className={cn(
                        'p-3 border-b flex items-center justify-center gap-4',
                        theme === 'dark' ? 'border-white/5' : 'border-gray-100'
                      )}
                    >
                      <button
                    ...
                    <div className="flex-1 p-4 flex items-center justify-center overflow-hidden">
                      <motion.div
                        ...
                        className={cn(
                          'w-full h-full shadow-2xl rounded-none border relative group overflow-hidden',
                          theme === 'dark' ? 'bg-white border-white/10' : 'bg-white border-gray-200'
                        )}
                      >
                        <iframe
                          ref={iframeRef}
                          src={`${import.meta.env.VITE_STOREFRONT_URL || 'http://localhost:5173'}?preview=true&pageId=${id || (isGlobal ? 'landing-page' : '')}`}
                  ...
                  <div className="w-full h-full p-4 overflow-y-auto custom-editor-scrollbar space-y-4">
                    {history.map((v, idx) => (
                      <div
                        key={v.id}
                        className={cn(
                          'group p-4 rounded-none border transition-all cursor-pointer space-y-2',
                          theme === 'dark'
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          >
                            {idx === 0 ? 'Current' : `V1.${history.length - idx}`}
                          </span>
                    ...
                    {field === 'description' ? (
                      <textarea
                        ...
                        onChange={(e) =>
                          ...
                          setData({
                            ...data,
                            meta: { ...(data.meta || {}), [field]: e.target.value },
                          } as any)
                        }
                        className={cn(
