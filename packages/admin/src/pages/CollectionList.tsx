  ...
  const handleBulkAction = async (action: 'delete' | 'publish' | 'unpublish') => {
    ...
    try {
      if (action === 'delete') {
        ...
        setData((prev) => prev.map((item) => selectedIds.has(item._id) ? { ...item, _status: 'published' } : item))
      } else if (action === 'unpublish') {
        await api.post(`/${slug}/bulk/unpublish`, { ids })
        toast.success(`${ids.length} records unpublished`)
        setData((prev) => prev.map((item) => selectedIds.has(item._id) ? { ...item, _status: 'draft' } : item))
      }
  ...
  return (
    <div className={cn('p-10 space-y-10 min-h-screen transition-colors duration-500', theme === 'dark' ? 'bg-black text-white' : 'bg-[#fafafa] text-gray-900')}>
      ...
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <Link to={`/collections/${slug}/new`} className={cn('px-8 py-4 rounded-none font-black text-[11px] uppercase tracking-widest shadow-xl transition-all italic leading-none flex items-center gap-3', theme === 'dark' ? 'bg-white text-black hover:bg-gray-200' : 'bg-indigo-600 text-white shadow-indigo-600/10')}>
        ...
        <div className={cn('px-6 py-4 border-b flex items-center justify-between', theme === 'dark' ? 'bg-white/[0.02] border-white/5' : 'bg-gray-50/20')}>
          <div className="relative w-full max-w-sm">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="NEURAL_SEARCH_KERNEL..."
              className={cn('w-full border rounded-none py-2.5 pl-10 pr-4 text-[9px] font-black italic focus:ring-4 transition-all outline-none uppercase tracking-widest', theme === 'dark' ? 'bg-black border-white/10 text-white focus:ring-indigo-500/20' : 'bg-white border-gray-100 focus:ring-indigo-500/10')}
            />
          </div>
          <div className="flex items-center gap-2 relative">
            ...
            <div className="relative">
              <button onClick={() => setColumnMenuOpen(!columnMenuOpen)} className="p-2.5 border rounded-none text-gray-500 hover:text-indigo-500 transition-colors">
                <Layers size={14} />
              </button>
              <AnimatePresence>
                {columnMenuOpen && (
            ...
            <button onClick={exportCSV} className="p-2.5 border rounded-none text-gray-500 hover:text-indigo-500 transition-colors" title="Export"><Download size={14} /></button>
            <button onClick={() => setImportModalOpen(true)} className="p-2.5 border rounded-none text-gray-500 hover:text-indigo-500 transition-colors" title="Import"><Upload size={14} /></button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
              ...
              <tr className={cn('border-b text-left text-[8px] font-black text-gray-500 uppercase tracking-[0.4em] italic', theme === 'dark' ? 'border-white/5' : 'border-gray-50')}>
                <th className="px-4 py-4 w-10">
                  <button onClick={(e) => { e.stopPropagation(); toggleSelectAll(); }} className="flex items-center justify-center" title={selectedIds.size === filteredData.length && filteredData.length > 0 ? 'Deselect all' : 'Select all'}>
                    {selectedIds.size === filteredData.length && filteredData.length > 0 ? <CheckSquare size={14} className="text-indigo-500" /> : <Square size={14} className={cn('text-gray-500', selectedIds.size > 0 && 'text-indigo-400')} />}
                  </button>
                </th>
            ...
            <tbody className={cn('divide-y', theme === 'dark' ? 'divide-white/5' : 'divide-gray-50')}>
              {loading ? (
                <tr><td colSpan={visibleColumns.length + 3} className="py-20 text-center"><Loader2 size={24} className="animate-spin mx-auto text-indigo-500 opacity-20" /></td></tr>
                ...
                return (
                  <tr key={item._id} className={cn("hover:bg-indigo-500/[0.02] transition-colors group cursor-pointer border-b border-white/[0.02]", isSelected && 'bg-indigo-500/[0.06]')}
                    onClick={() => navigate(`/collections/${slug}/${item._id}`)}
                  >
                    <td className="px-4 py-4">
                      <button onClick={(e) => { e.stopPropagation(); toggleSelect(item._id); }} className="flex items-center justify-center">
                        {isSelected ? <CheckSquare size={14} className="text-indigo-500" /> : <Square size={14} className="text-gray-600 group-hover:text-gray-400" />}
                      </button>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                        <button onClick={(e) => { e.stopPropagation(); }} className="p-2 rounded-none border hover:text-indigo-500"><Edit size={12} /></button>
