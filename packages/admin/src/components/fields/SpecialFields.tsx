...
const PointField: React.FC<Props> = ({ field: _field, value, onChange, disabled }) => {
 ...
 step="any"
 disabled={disabled}
 className="w-full bg-white/[0.05] backdrop-blur-md border border-white/[0.08] rounded-none px-3 py-2 text-sm focus:border-emerald-500/50 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:ring-offset-1 focus-visible:ring-offset-black disabled:opacity-60 disabled:cursor-not-allowed text-white placeholder:text-gray-500"
 placeholder="0.0"
 />
 </div>
 <div className="flex-1 space-y-1">
...
const JoinField: React.FC<{ field: { collection?: string } }> = ({ field }) => (
 <div className="w-full bg-white/[0.03] border border-white/[0.08] rounded-none px-4 py-3 text-sm text-gray-400 flex items-center gap-2">
...
const RadioField: React.FC<Props> = ({ field, value, onChange, disabled }) => {
 ...
 disabled={disabled}
 className="w-4 h-4 border border-white/[0.08] text-emerald-600 dark:text-emerald-500 focus:ring-emerald-500 disabled:opacity-60 accent-emerald-500"
 />
