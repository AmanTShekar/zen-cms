...
const JoinField: React.FC<{ field: { collection?: string } }> = ({ field }) => (
  <div className="w-full bg-white/[0.03] border border-white/[0.08] rounded-none px-4 py-3 text-sm text-gray-400 italic flex items-center gap-2">
        ...
        return (
          <label key={`${String(optVal)}-${i}`} className="flex items-center gap-2.5 cursor-pointer group">
            <input
              ...
              disabled={disabled}
              className="w-4 h-4 border border-white/[0.08] text-emerald-500 focus:ring-emerald-500 disabled:opacity-60 accent-emerald-500"
            />
