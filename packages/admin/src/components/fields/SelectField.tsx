...
const SelectField: React.FC<Props> = ({ field, value, onChange, disabled }) => {
  ...
  return (
    <select
      ...
      disabled={disabled}
      className={cn(
        "w-full px-4 py-2.5 text-xs transition-all duration-350 outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:ring-offset-1 focus-visible:ring-offset-black",
        "backdrop-blur-md",
        theme === 'dark'
