...
export default function RecentContentWidget({ config = {}, theme, title }: WidgetProps) {
  ...
  return (
    <div className="h-full flex flex-col gap-3">
      ...
      {loading && <p className="text-[9px] text-gray-500 italic">Loading...</p>}
      <div className="space-y-2 overflow-y-auto flex-1">
        {logs.length === 0 && !loading && (
          <p className="text-[9px] text-gray-500 italic text-center py-6">
        ...
        {logs.map((log: any, i: number) => (
          ...
          >
            ...
            <ArrowRight
              size={12}
              className="text-gray-500 group-hover:text-indigo-500 transition-colors shrink-0"
            />
          </button>
        ))}
      </div>
