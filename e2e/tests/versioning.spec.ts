...
test.describe('Versioning & Drafts', () => {
  ...
  test('Save as Draft and Verify Public API Exclusion', async ({ page, request }) => {
    ...
    if (await createBtn.isVisible()) {
      ...
      const draftToggle = page.locator('button[role="switch"], input[type="checkbox"]').filter({ hasText: /draft/i });
  ...
  test('Revert to a previous version', async ({ page }) => {
    ...
    if (await editLink.isVisible()) {
      ...
      const versionsBtn = page.locator('button:has-text("Versions"), button[title="Version History"]');
