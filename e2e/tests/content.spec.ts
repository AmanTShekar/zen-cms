...
test.describe('Content Editing (CRUD)', () => {
  ...
  test('Create a new document', async ({ page }) => {
    ...
    await page.getByPlaceholder('Enter Title...').first().fill('E2E Test Document ' + uniqueSuffix);
    ...
    // 3. Edit the document
    await page.getByPlaceholder('Enter Title...').first().fill('E2E Test Document ' + uniqueSuffix + ' - Updated');
    await page.getByRole('button', { name: /Publish/i }).first().click();
    await page.getByRole('button', { name: /Publish Now/i }).click();
    await page.waitForTimeout(2000);
    ...
    await expect(page.getByText('E2E Test Document ' + uniqueSuffix + ' - Updated').first()).toBeVisible();
    ...
    // Check it's gone
    await expect(page.getByText('E2E Test Document ' + uniqueSuffix + ' - Updated').first()).not.toBeVisible();
  });
